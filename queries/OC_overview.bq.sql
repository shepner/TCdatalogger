...
SELECT
  ...
  (SELECT COUNT(DISTINCT slots_position)
   FROM `torncity-402423.torn_data.v2_faction_40832_crimes` c2
   WHERE c2.id = sci.id) as `Positions`,
  ROUND(AVG(c.rewards_money), 0) as `Average reward`,
  ROUND(AVG(c.rewards_money) / NULLIF((SELECT COUNT(DISTINCT slots_position)
   FROM `torncity-402423.torn_data.v2_faction_40832_crimes` c2
